...
describe("uint_pack function", () => {
    test("pack a byte with a byte offset", () => {
        ...
        const bit_length = uint_pack(0b00101010, {bits: 8, byte_offset: 2, data_view: new DataView(bytes.buffer)});
    ...
    test("pack a uint16 little endian", () => {
        ...
        const bit_length = uint_pack(0b0001111111000000, {bits: 16, byte_offset: 0, data_view: data_view, little_endian: true});
    ...
    test("pack a uint32 big endian", () => {
        ...
        const data_view = new DataView(buffer);
        const bit_length = uint_pack(0b00000001111111111111000000000000, {bits: 32, byte_offset: 0, data_view: data_view, little_endian: undefined});
    ...
    test("pack a uint64 little endian", () => {
        ...
        const data_view = new DataView(buffer);
        const now = Date.now();
        const bit_length = uint_pack(now, {bits: 64, byte_offset: 0, data_view: data_view, little_endian: true});
    ...
    test("pack a uint16 with too large of a value throws an error", () => {
        expect(() => {uint_pack(33550336, {bits: 16, byte_offset: 0, data_view: new DataView(new ArrayBuffer(2))})})
            .toThrow(/Unable to encode/);
    });
    test("pack a uint16 with not an integer throws an error", () => {
        expect(() => {uint_pack(42.42, {bits: 16, byte_offset: 0, data_view: new DataView(new ArrayBuffer(2))})})
            .toThrow(/Unable to encode/);
    });
    test("pack a uint32 with an unsafe integer throws an error", () => {
        expect(() => {uint_pack(Number.MAX_SAFE_INTEGER + 1, {bits: 32, byte_offset: 0, data_view: new DataView(new ArrayBuffer(4))})})
            .toThrow(/Unable to encode/);
    });
});
describe("int_pack function", () => {
    test("pack a byte with a byte offset", () => {
        ...
        const bit_length = int_pack(-42, {bits: 8, byte_offset: 2, data_view: new DataView(bytes.buffer)});
        expect(bit_length).toEqual(8);
        expect(Array.from(bytes)).toEqual([0, 1, -42, 3]);
    });
    test("pack a uint16 little endian", () => {
    ...
    test("pack a int16 with too large of a value throws an error", () => {
        expect(() => {int_pack(65536, {bits: 16, byte_offset: 0, data_view: new DataView(new ArrayBuffer(2))})})
            .toThrow(/Unable to encode/);
    });
    test("pack a int16 with not an integer throws an error", () => {
        expect(() => {int_pack(42.42, {bits: 16, byte_offset: 0, data_view: new DataView(new ArrayBuffer(2))})})
            .toThrow(/Unable to encode/);
    });
});
describe("float_pack function", () => {
    test("pack a float64 little endian", () => {
        const buffer = new ArrayBuffer(8);
        const data_view = new DataView(buffer);
        const bit_length = float_pack(8128.8128, {bits: 64, byte_offset: 0, data_view: data_view, little_endian: true});
        expect(bit_length).toEqual(64);
    ...
    test("pack a float32 big endian with a byte offset", () => {
        ...
        const bit_length = float_pack(-42.42, {bits: 32, byte_offset: 8, data_view: data_view});
        expect(bit_length).toEqual(32);
    ...
    test("pack a float32 little endian with a bit offset", () => {
        ...
        const data_view = new DataView(buffer);
        const bit_offset = 3;
        const bit_length = float_pack(-41.41, {bits: 32, byte_offset: bit_offset / 8, data_view: data_view, little_endian: true});
        expect(bit_length).toEqual(32);
...
describe("uint_parse function", () => {
    test("parse a byte with a byte offset", () => {
        ...
        expect(uint_parse({bits: 8, byte_offset: 2, data_view: new DataView(bytes.buffer)}))
            .toEqual(42);
    });
    test("parse a uint16 little endian", () => {
        ...
        expect(uint_parse({bits: 16, byte_offset: 0, data_view: data_view, little_endian: true}))
            .toEqual(8128);
    });
    test("parse a uint32 big endian", () => {
        ...
        expect(uint_parse({bits: 32, byte_offset: 0, data_view: data_view, little_endian: undefined}))
            .toEqual(33550336);
    });
    test("parse a uint64 little endian", () => {
        ...
        expect(uint_parse({bits: 64, byte_offset: 0, data_view: data_view, little_endian: true}))
            .toEqual(now);
    });
    test("parse a uint16 big endian with a bit offset", () => {
        ...
        expect(uint_parse({bits: 16, byte_offset: bit_offset / 8, data_view: data_view}))
            .toEqual(value);
    });
    test("parse uint1-7 with no shift", () => {
        const data_view = new DataView(new Uint8Array([0xFF]).buffer);
        expect(uint_parse({bits: 1, byte_offset: 0, data_view})).toEqual(1);
        expect(uint_parse({bits: 2, byte_offset: 0, data_view})).toEqual(3);
        expect(uint_parse({bits: 3, byte_offset: 0, data_view})).toEqual(7);
        expect(uint_parse({bits: 4, byte_offset: 0, data_view})).toEqual(15);
        expect(uint_parse({bits: 5, byte_offset: 0, data_view})).toEqual(31);
        expect(uint_parse({bits: 6, byte_offset: 0, data_view})).toEqual(63);
        expect(uint_parse({bits: 7, byte_offset: 0, data_view})).toEqual(127);
    });
    test("parse a uint2 with a bit shift without byte boundary", () => {
        const buffer = new ArrayBuffer(1);
        const data_view = new DataView(buffer);
        data_view.setUint8(0, 0xAA);
        expect(uint_parse({bits: 2, byte_offset: 5/8, data_view: data_view}))
            .toEqual(1);
        expect(uint_parse({bits: 2, byte_offset: 4/8, data_view: data_view}))
            .toEqual(2);
    });
    test("parse a uint3 with a bit shift with a byte boundary", () => {
        ...
        expect(uint_parse({bits: 3, byte_offset: 6/8, data_view: data_view}))
            .toEqual(7);
        expect(uint_parse({bits: 3, byte_offset: 5/8, data_view: data_view}))
            .toEqual(6);
        expect(uint_parse({bits: 3, byte_offset: 4/8, data_view: data_view}))
            .toEqual(4);
        expect(uint_parse({bits: 3, byte_offset: 7/8, data_view: data_view}))
            .toEqual(3);
    });
    test("parse a uint64 with an unsafe integer throws an error", () => {
        ...
        const data_view = new DataView(buffer);
        expect(() => {uint_parse({bits: 64, byte_offset: 0, data_view: data_view})})
...
describe("int_parse function", () => {
    test("parse a byte with a byte offset", () => {
        ...
        expect(int_parse({bits: 8, byte_offset: 2, data_view: new DataView(bytes.buffer)}))
            .toEqual(-42);
    });
    test("parse a int16 little endian", () => {
        ...
        const data_view = new DataView(buffer);
        data_view.setUint16(0,0b0001111111000000, true);
        expect(int_parse({bits: 16, byte_offset: 0, data_view: data_view, little_endian: true}))
            .toEqual(8128);
    });
    test("parse a int32 big endian", () => {
        ...
        expect(int_parse({bits: 32, byte_offset: 0, data_view: data_view, little_endian: undefined}))
            .toEqual(-33550336);
    });
    test("parse a int16 big endian with a bit offset", () => {
        ...
        data_view.setUint8(1, (0xAF >> (8 - bit_offset)) | ((0x0F << bit_offset) & 0xFF));
        data_view.setUint8(2, (0x0F >> (8 - bit_offset)));
        expect(int_parse({bits: 16, byte_offset: bit_offset / 8, data_view: data_view}))
            .toEqual(-(2**16 - value));
    });
});
describe("float_parse function", () => {
    test("parse a float64 big endian", () => {
        ...
        expect(float_parse({bits: 64, byte_offset: 0, data_view: data_view}))
            .toBeCloseTo(8128.8128);
    });
    test("parse a float32 little endian with a byte offset", () => {
        const bytes = new Float32Array([0.1, 0.2, -42.42, 0.4]);
        expect(float_parse({bits: 32, byte_offset: 8, data_view: new DataView(bytes.buffer), little_endian: true}))
            .toBeCloseTo(-42.42);
    });
    test("parse a float32 big endian with a bit offset", () => {
        ...
        expect(float_parse({bits: 32, byte_offset: bit_offset / 8, data_view: data_view}))
            .toBeCloseTo(value);
    });
});
describe("utf8_pack function", () => {
    test("pack an unsized string with a byte offset", () => {
        const buffer = new ArrayBuffer(8);
        const data_view = new DataView(buffer);
        const bit_length = utf8_pack('💩', {bits: 0, byte_offset: 3, data_view});
        expect(bit_length).toEqual(32);
    ...
    test("pack a string with a bit offset (but, really, don't do this in practice)", () => {
        ...
        const data_view = new DataView(buffer);
        const bit_offset = 3;
        const bit_length = utf8_pack(value, {bits: 32, byte_offset: bit_offset / 8, data_view: data_view});
        expect(bit_length).toEqual(32);
        const tmp_buffer = new ArrayBuffer(5);
        const tmp_view = new DataView(tmp_buffer);
    ...
    test("pack a string with a too short of bits for string serialization throws an error", () => {
        expect(() => {utf8_pack('🚲', {bits: 16, byte_offset: 0, data_view: new DataView(new ArrayBuffer(4))})})
    ...
    test("pack a string with an offset that won't fit in the buffer throws an error", () => {
        expect(() => {utf8_pack('🚲', {bits: 0, byte_offset: 2, data_view: new DataView(new ArrayBuffer(4))})})
...
describe("utf8_parse function", () => {
    test("parse a string with a byte offset", () => {
        ...
        const data_view = new DataView(buffer);
        expect(utf8_parse({bits: 32, byte_offset: 3, data_view: data_view}))
            .toEqual('💩');
    });
    test("parse a string with a bit offset (but, really, don't do this in practice)", () => {
        ...
        data_view.setUint8(3, (bytes[2] >> (8 - bit_offset)) | ((bytes[3] << bit_offset) & 0xFF));
        data_view.setUint8(4, (bytes[3] >> (8 - bit_offset)));
        expect(utf8_parse({bits: 32, byte_offset: bit_offset / 8, data_view: data_view}))
            .toEqual('🚲');
    });
});
