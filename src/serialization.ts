...
const write_bit_shift: ((packer: Serializer<any>, value: any, options: Serialization_Options) => Size) =
        (packer, value, {size, data_view, byte_offset, little_endian}) => {
    /*
    bit_offset = 5
    buffer = 00011111
    byte = xxxxxxxx
    ...
    const bit_length = packer(value, {size: size, byte_offset: 0, data_view: new DataView(bytes.buffer), little_endian});
...
const read_bit_shift: ((parser: Deserializer<any>, options: Serialization_Options) => any) =
    (parser, {size, data_view, byte_offset, little_endian}) => {
        const bit_offset = (byte_offset % 1) * 8;
        byte_offset = Math.floor(byte_offset);
        const bytes = new Uint8Array(Math.ceil(size / 8));
        ...
        return parser({size: size, byte_offset: 0, data_view: new DataView(bytes.buffer), little_endian});
    };

export const uint_pack: Serializer<number> = (value, {size, data_view, byte_offset, little_endian}) => {
    ...
    if (byte_offset % 1) {
        return write_bit_shift(uint_pack, value, {size, data_view, byte_offset, little_endian});
    } else {
        switch (size) {
            case 1:
            case 2:
...
export const uint_parse: Deserializer<number> = ({size, data_view, byte_offset, little_endian}) => {
    if (byte_offset % 1) {
        return read_bit_shift(uint_parse, {size, data_view, byte_offset, little_endian});
    } else {
        switch (size) {
            case 1:
            case 2:
...
export const int_pack: Serializer<number> = (value, {size, data_view, byte_offset, little_endian}) => {
    const original_value = value;
    value = Math.floor(original_value);
    ...
    if (byte_offset % 1) {
        return write_bit_shift(int_pack, value, {size, data_view, byte_offset, little_endian});
    } else {
        switch (size) {
            case 8:
                data_view.setUint8(byte_offset, value);
...
export const int_parse: Deserializer<number> = ({size, data_view, byte_offset, little_endian}) => {
    if (byte_offset % 1) {
        return read_bit_shift(int_parse, {size, data_view, byte_offset, little_endian});
    } else {
        switch (size) {
            case 8:
...
export const float_pack: Serializer<number> = (value, {size, data_view, byte_offset, little_endian}) => {
    /* TODO: Input validation */
    if (byte_offset % 1) {
        return write_bit_shift(float_pack, value, {size, data_view, byte_offset, little_endian});
    } else {
        switch (size) {
            case 32:
...
export const float_parse: Deserializer<number> = ({size, data_view, byte_offset, little_endian}) => {
    if (byte_offset % 1) {
        return read_bit_shift(float_parse, {size, data_view, byte_offset, little_endian});
    } else {
        switch (size) {
            case 32:
...
export const utf8_pack: Serializer<string> = (value, {size, data_view, byte_offset}) => {
    if (byte_offset % 1) {
        return write_bit_shift(utf8_pack, value, {size, data_view, byte_offset});
    } else {
...
export const utf8_parse: Deserializer<string> = ({size, data_view, byte_offset}) => {
    if (byte_offset % 1) {
        return read_bit_shift(utf8_parse, {size, data_view, byte_offset});
    } else {
        return utf8_decoder.decode(new DataView(data_view.buffer, byte_offset, size ? size / 8 : undefined));
    }
};
